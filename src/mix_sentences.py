...
# 단일 문서 -> sentence_list
# 특수문자 제거 전처리도 함. arg로 설정
def split_to_sentences(context):#, data_args):    
    # kiwi = Kiwi()
    sentence_list = []
    sentences = kiwi.split_into_sents(context)
    
    # if data_args.erase_punc:
    for sentence in sentences:
        sentence = re.sub(r'n\\n', '', sentence.text)
        # sentence_list.append(re.sub(r'[@%\\*=()~#&\+á?\xc3\xa1\-\|\.\:\;\!\-\,\_\~\$\'\"]', '', sentence))
...
def mix_sentences(examples, ratio):#, data_args):
    # ratio = data_args.mix_ratio
    ...
    for i in tqdm(range(len(contexts))):
        ...
        # while mix_idx == sorted_mix_idx: # 안 섞인 상태면 섞일 때까지 돌리기
...
# test = load_from_disk("/opt/ml/input/level2-mrc-level2-nlp-02/src/mixed_datasets")
# print(test)
# print(test['train'][10])
"""
# Upload the dataset to the Hub
## 커맨드에 이거 치고 huggingface-cli login
mix_dataset = mix_sentences(datasets['train'], 0.1)
mix_dataset.push_to_hub("salt-bread/mixed_wiki", private=True)
"""